...
def listar_quizzes_professor(professor_id: str) -> List[Dict[str, Any]]:
    ...
    try:
        if not _eh_uuid_valido(professor_id):
            return []
            
        res = (
            banco.table("quizzes")
            .select("id, titulo")
            .eq("criado_por", str(professor_id).strip())
            .order("data_criacao", desc=True)
            .execute()
        )
        return res.data or []
    except Exception as exc:
        print(f"Erro ao listar quizzes: {exc}")
        return []

# Atualize a função de criar_quiz para dar um tratamento de erro amigável
def criar_quiz(titulo: str, professor_id: str, disciplina_nome: str = "", tema: str = "") -> Dict[str, Any]:
    ...
    if len(titulo) < 3:
        return _erro("O título do quiz deve ter pelo menos 3 caracteres.")

    permitido, motivo = _verificar_permissao_professor(professor_id)
    if not permitido:
        return _erro(f"Acesso negado: {motivo}")

    # Resolve o ID da disciplina dinamicamente (reutilizando a lógica do monólito)
    disciplina_id = None
    ...
    novo_quiz = {
        ...
        "status": "criado",
        "pergunta_atual": 0,
        "disciplina_id": disciplina_id,  # Vinculação da disciplina (Item 4.74)
        "tema": tema.strip() if tema else None  # Vinculação de assunto/tema (Item 4.75)
    }

    try:
        dados = repo_insert_quiz(novo_quiz)
        if dados:
            return _ok(dados, "Quiz configurado e registrado com sucesso!")
        return _erro("Não foi possível registrar o quiz no servidor.")
    except Exception as exc:
        if "42501" in str(exc):
