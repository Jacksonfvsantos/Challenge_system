...
def cadastrar_questao_rapida(enunciado, alternativas_texto, indice_correta):
    """
    Cadastra uma questão preenchendo a coluna obrigatória 'indice_correto' 
    e insere suas 4 alternativas vinculadas.
    """
    try:
        # 1. Insere o enunciado preenchendo a coluna exigida pelo banco
        res_q = supabase.table("questoes").insert({
            "enunciado": enunciado.strip(),
            "indice_correto": int(indice_correta) # ✅ Preenche a restrição NOT NULL do seu banco
        }).execute()
        ...
        # 2. Mantém o salvamento das alternativas na tabela relacionada
        lote_alternativas = []
