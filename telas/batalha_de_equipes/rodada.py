...
def obter_nomes_dos_times(time_a_id, time_b_id):
    ...
    except Exception: return "Time A", "Time B"
...
def obter_pergunta_atual(batalha_id, ordem_pergunta):
    try:
        ...
        if not vinculo.data: return None
        ...
        if res.data:
            for r in res.data:
                if r.get("resposta_correta") is True:
                    if str(r.get("time_id")).strip() == str(time_a_id).strip(): pa += 1
                    elif str(r.get("time_id")).strip() == str(time_b_id).strip(): pb += 1
        return pa, pb
...
def tela_batalha_rodada():
    ...
    if tipo == "aluno":
        ...
        if not tid: st.error("Você precisa de uma equipe!"); return
    else: tid, tnome = "PROFESSOR_CONSOLA", "Painel Docente"
    
    if "batalha_ativa_id" not in st.session_state: return
    ...
    if b.get("finalizada") is True or str(b.get("status")).lower() == "finalizada" or not dados_p:
        ...
        if pa > pb: st.info(f"🥇 **Vencedor:** {nome_ta} ({pa} XP) | 🥈 {nome_tb} ({pb} XP)")
        elif pb > pa: st.info(f"🥇 **Vencedor:** {nome_tb} ({pb} XP) | 🥈 {nome_ta} ({pa} XP)")
        else: st.warning(f"🤝 **Resultado:** Empate técnico ({pa} XP)")
        
        if st.button("⬅️ Voltar para a Arena de Equipes", use_container_width=True):
            st.session_state.pagina = "batalha_de_equipes"; st.rerun()
        return
    ...
    if tipo == "aluno" and str(tid) in (ta_id, tb_id):
        if eh_vez: st.markdown(f"<div style='background-color: #065f46; padding: 12px; border-radius: 5px; color: white;'>🟢 SEU TIME RESPONDE AGORA! (Tentativa {tentativa}ª)</div>", unsafe_allow_html=True)
        else: st.markdown("<div style='background-color: #7c2d12; padding: 12px; border-radius: 5px; color: white;'>⏱️ ADVERSÁRIO A RESPONDER...</div>", unsafe_allow_html=True)
