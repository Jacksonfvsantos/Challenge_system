...
def formatar_data_br(data_str):
    """Converte datas ISO (aaaa-mm-dd) para o padrão brasileiro (dd/mm/aaaa)."""
    if not data_str: 
        return "Sem prazo"
    try:
...
def tela_batalha_gerenciar():
    ...
    # Inicialização de estados para limpeza total automática pós-sucesso
    ...
    with st.container(border=True):
        ...
        with st.expander("🛡️ Configurações de Segurança Avançada", expanded=False):
            ...
            with col3: 
                bloquear_copia = st.checkbox("Bloquear cópia de código", value=True)
            with col4: 
                verificar_plagio = st.checkbox("Mecanismo anti-plágio ativo", value=True)
            with col5: 
                limitar_ip = st.checkbox("Limitar por endereço IP", value=False)
        ...
        if st.button("Gravar e Lançar Batalha", use_container_width=True):
            if not titulo.strip():
                ...
                if titulo.strip().lower() in titulos_existentes:
                    ...
                    if criar_batalha(titulo.strip(), descricao, user_id, quantidade_rodadas, tempo_por_rodada, criterios, regras, seguranca, prazo_iso):
                        st.success("✅ Batalha criada e disponibilizada com sucesso!")
    ...
    for b in batalhas:
        ...
        # Exibição formatada em padrão brasileiro
        prazo_br = formatar_data_br(prazo_original)

        # Validação temporal retroativa
        expirada = False
        if prazo_original:
            try:
        ...
        with st.container(border=True):
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <strong style="color:#0d1b2a; font-size:16px;">{b.get('titulo', 'Sem Título')}</strong>
            ...
            if b.get("regras_conduta"):
                with st.expander("Regras Adicionais"):
                    st.write(b["regras_conduta"])

            if expirada:
                st.error("🔒 Modificações bloqueadas: O prazo final estabelecido para esta batalha expirou.")
            elif not finalizada:
