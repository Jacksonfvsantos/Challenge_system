...
def tela_cadastro_perguntas_quiz():
    ...
    # Mostra o status atual do caderno de perguntas
    ...
    # 2. Abas de Input (Manual vs Arquivo)
    ...
    with aba_manual:
        ...
        with st.form("form_nova_pergunta_manual", clear_on_submit=True):
            ...
            # Escolha da Correta usando rótulos claros
    ...
    with aba_import:
        ...
        if arquivo_enviado is not None:
            ...
            if st.button("🧠 Iniciar Extração e Mapear Questões", type="primary", use_container_width=True):
                # Aqui entrará a chamada do seu motor de parsing de texto (ex: PyPDF2 ou python-docx)
                # integrado com a chamada sequencial do cadastrar_pergunta_completa.
    ...
    # 3. Lista de visualização rápida das questões já salvas no rodapé
    if perguntas_atuais:
