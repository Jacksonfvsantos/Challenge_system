...
def tela_central_regras():
    ...
    with aba_provas:
        ...
        with st.container(border=True):
            ...
            st.markdown("""
            - Each questão respondida corretamente concede **1 ponto** na nota base da avaliação.
    ...
    with aba_quiz:
        ...
        with st.container(border=True):
            st.markdown("**⭐ Critérios de Pontuação**")
            st.markdown("""
            - Each resposta correta adiciona **+10 pontos** imediatos ao score do aluno no Quiz.
